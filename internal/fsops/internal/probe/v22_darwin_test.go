...
func TestV22(t *testing.T) {
	...
	check := func(t *testing.T, label, dir string) {
		...
		setup := func(c string) string {
			...
			logf("%s: after setxattr: a=%+q", c, testfs.ListRawNames(t, filepath.Join(d, "a")))
			return d
		}
		// 1. x だけをリネームする。
		d := setup("rename-main")
		...
		logf("rename-main: rename a/x -> b/x: err=%v; a=%+q b=%+q xattr=%s", errString(err),
			testfs.ListRawNames(t, filepath.Join(d, "a")), testfs.ListRawNames(t, filepath.Join(d, "b")), xattrOf(filepath.Join(d, "b", "x")))
		...
		logf("rename-companion-first: rename a/._x -> b/._x: err=%v; b=%+q xattr(a/x)=%s", errString(err),
			testfs.ListRawNames(t, filepath.Join(d, "b")), xattrOf(filepath.Join(d, "a", "x")))
		err = os.Rename(filepath.Join(d, "a", "x"), filepath.Join(d, "b", "x"))
		...
		err = os.Rename(filepath.Join(d, "a", "x"), filepath.Join(d, "b", "x"))
		logf("rename-main-first: rename a/x -> b/x: err=%v; a=%+q", errString(err), testfs.ListRawNames(t, filepath.Join(d, "a")))
		...
		logf("rename-plain-onto-stale: before: a=%+q b=%+q", testfs.ListRawNames(t, filepath.Join(d, "a")), testfs.ListRawNames(t, filepath.Join(d, "b")))
		err = os.Rename(filepath.Join(d, "a", "y"), filepath.Join(d, "b", "y"))
		logf("rename-plain-onto-stale: rename a/y -> b/y: err=%v; b=%+q", errString(err), testfs.ListRawNames(t, filepath.Join(d, "b")))
		// 6. x を削除する。
		d = setup("unlink-main")
		err = unix.Unlink(filepath.Join(d, "a", "x"))
		logf("unlink-main: unlink a/x: err=%v; a=%+q", errString(err), testfs.ListRawNames(t, filepath.Join(d, "a")))
		// 7. ._x を削除する。
		d = setup("unlink-companion")
		err = unix.Unlink(filepath.Join(d, "a", "._x"))
		logf("unlink-companion: unlink a/._x: err=%v; a=%+q xattr=%s", errString(err), testfs.ListRawNames(t, filepath.Join(d, "a")), xattrOf(filepath.Join(d, "a", "x")))
		...
		logf("rmdir: Lsetxattr f: err=%v; names=%+q", errString(err), testfs.ListRawNames(t, d))
		err = unix.Rmdir(filepath.Join(d, "f"))
		logf("rmdir: rmdir f: err=%v; names=%+q", errString(err), testfs.ListRawNames(t, d))
	}
