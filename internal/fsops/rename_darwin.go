...
func renameExclusiveSys(s, d string) error {
	err := ignoringEINTR(func() error { return unix.RenamexNp(s, d, unix.RENAME_EXCL) })
...
func renameAtExclusiveSys(sfd int, s string, dfd int, d string) error {
	err := ignoringEINTR(func() error { return unix.RenameatxNp(sfd, s, dfd, d, unix.RENAME_EXCL) })
	if errors.Is(err, unix.ENOTSUP) {
