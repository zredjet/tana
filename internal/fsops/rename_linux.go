...
func renameExclusiveSys(s, d string) error {
	err := ignoringEINTR(func() error { return unix.Renameat2(unix.AT_FDCWD, s, unix.AT_FDCWD, d, unix.RENAME_NOREPLACE) })
...
func renameAtExclusiveSys(sfd int, s string, dfd int, d string) error {
	err := ignoringEINTR(func() error { return unix.Renameat2(sfd, s, dfd, d, unix.RENAME_NOREPLACE) })
	if errors.Is(err, unix.EINVAL) || errors.Is(err, unix.ENOSYS) {
