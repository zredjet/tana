...
func disallowedDeps(deps []string, self string) []string {
	...
	for _, line := range deps {
		// テスト用の変種は "p [p.test]"、テストの main パッケージは "p.test" と表示される。
		p, _, _ := strings.Cut(strings.TrimSpace(line), " ")
		if p == "" {
			continue
		}
		if within(p, self) || within(strings.TrimSuffix(p, ".test"), self) {
			continue
		}
...
// TestDependencies は、対応するすべての GOOS と CGO_ENABLED の組み合わせで、
...
func TestDependencies(t *testing.T) {
	...
	for _, goos := range []string{"windows", "darwin", "linux"} {
		for _, cgo := range []string{"0", "1"} {
			env := []string{"GOOS=" + goos, "GOARCH=amd64", "CGO_ENABLED=" + cgo}
			t.Run(goos+"/cgo="+cgo, func(t *testing.T) {
				t.Parallel()
